/*:
 * @plugindesc L3 Net Core v0.2 - Sessions locales et distantes 2-4 joueurs pour RPG Maker MV.
 * @author L3ViKk
 * @version 0.2.0
 *
 * @param Enabled
...
(function() {
    ...
    var config = {
        ...
        sendInterval: Math.max(1, readNumber("Position Send Interval", 6)),
    ...
    function handlePacket(raw) {
        ...
        if (type === "welcome") {
            ...
            setRemoteState(data.playerId, data.name, data.state);
    ...
    function setRemoteState(playerId, name, state) {
        if (!playerId || playerId === Net.state.playerId) return;
        var player = Net.state.players[playerId] || { playerId: playerId };
    ...
    function sendPlayerState(force) {
        ...
        if (!force && Graphics.frameCount % config.sendInterval !== 0) return;
        Net.send("playerState", localPlayerState());
    }
    ...
    Game_L3NetRemote.prototype.applyPlayer = function(player) {
        ...
        if (!this._l3Ready || previousMapId !== mapId || Math.abs(this._x - targetX) > 8 || Math.abs(this._y - targetY) > 8) {
            this.locate(targetX, targetY);
            this._l3Ready = true;
        } else {
            this._x = Math.round(targetX);
            this._y = Math.round(targetY);
        }
    };
    ...
    function updateSpriteset(spriteset) {
        ...
        for (var playerId in players) {
            ...
            character.update();
            updateNameplate(spriteset._l3NetSprites[playerId], character);
        }
