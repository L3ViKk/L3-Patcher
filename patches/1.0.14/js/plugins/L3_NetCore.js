/*:
 * @plugindesc L3 Net Core v0.6.0 - Lobby titre et sessions distantes 2-4 joueurs pour RPG Maker MV.
 * @author L3ViKk
 * @version 0.6.0
 *
 * @param Enabled
 ...
 * @default true
 *
 * @param Enable Title Lobby
 * @text Lobby sur l'ecran titre
 * @type boolean
 * @default true
 *
 * @param Title Lobby Label
 * @text Libelle titre
 * @type text
 * @default Multijoueur
 *
...
(function() {
    ...
    var config = {
        ...
        titleLobby: readBool("Enable Title Lobby", true),
        titleLobbyLabel: readText("Title Lobby Label", "Multijoueur"),
    ...
    Net.state.jitter = 0;
    Net.state.quality = "-";
    Net.state.isHost = false;
    Net.state.roomMeta = null;
    Net.state.pendingGameStart = null;
    Net.lobby = Net.lobby || {};
    Net.lobby.serverUrl = storageGet("l3-net-lobby-url", Net.state.remoteUrl || config.remoteUrl);
    Net.lobby.sessions = [];
    Net.lobby.status = "idle";
    Net.lobby.lastError = "";
    Net.lobby.lastRefreshAt = 0;
    Net.lobby.selectedRoomId = "";
    Net.lobby.pendingHostRoom = null;
    ...
    function urlWithRoom(url, roomCode) {
        ...
        if (/[?&]room=/.test(url)) return url.replace(/([?&])room=[^&]*/, "$1room=" + encodeURIComponent(roomCode));
        return url + (url.indexOf("?") === -1 ? "?" : "&") + "room=" + encodeURIComponent(roomCode);
    }

    function urlWithoutRoom(url) {
        url = cleanUrl(url);
        ...
        var query = url.slice(queryIndex + 1).split("&").filter(function(part) {
            return part && part.split("=")[0] !== "room";
        }).join("&");
    ...
    function cleanSessionTitle(title) {
        return String(title || "").replace(/^\s+|\s+$/g, "").replace(/\s+/g, " ").slice(0, 48);
    }

    function makeRoomCode() {
        return normalizeRoom("L3" + Math.random().toString(36).slice(2, 7));
    }

    function roomsHttpUrl(url) {
        url = cleanUrl(url || Net.lobby.serverUrl || Net.state.remoteUrl || config.remoteUrl);
        url = url.replace(/^wss:\/\//, "https://").replace(/^ws:\/\//, "http://");
        url = url.replace(/\?room=.*$/, "").replace(/\/$/, "");
        return url + "/rooms";
    }
    ...
    function httpJson(url, callback) {
        try {
            var xhr = new XMLHttpRequest();
            xhr.open("GET", url + (url.indexOf("?") === -1 ? "?" : "&") + "v=" + Date.now(), true);
            xhr.overrideMimeType("application/json");
            xhr.timeout = 5000;
            xhr.onload = function() {
                if (xhr.status >= 400) {
                    callback(new Error("Serveur lobby indisponible."));
                    return;
                }
                try {
                    callback(null, JSON.parse(xhr.responseText || "{}"));
                } catch (error) {
                    callback(new Error("Reponse lobby invalide."));
                }
            };
            xhr.onerror = function() {
                callback(new Error("Impossible de joindre le serveur lobby."));
            };
            xhr.ontimeout = function() {
                callback(new Error("Timeout serveur lobby."));
            };
            xhr.send();
        } catch (error2) {
            callback(error2);
    ...
    function normalizeSessions(rooms) {
        var result = [];
        for (var i = 0; i < rooms.length; i++) {
            var room = rooms[i] || {};
            var maxPlayers = Math.max(2, Math.min(4, Number(room.maxPlayers || config.maxPlayers || 4)));
            var players = Math.max(0, Number(room.players || 0));
            var status = String(room.status || "waiting");
            if (players >= maxPlayers) status = "full";
            result.push({
                roomId: normalizeRoom(room.roomId || room.id || config.defaultRoomCode),
                title: cleanSessionTitle(room.title || room.name || room.roomId || "Partie"),
                hostName: cleanPlayerName(room.hostName || "Hote"),
                hostId: String(room.hostId || ""),
                status: status,
                maxPlayers: maxPlayers,
                players: players,
                ready: Number(room.ready || 0),
                slots: Math.max(0, maxPlayers - players),
    ...
    function selectedSession() {
        var sessions = Net.lobby.sessions || [];
        for (var i = 0; i < sessions.length; i++) {
            if (sessions[i].roomId === Net.lobby.selectedRoomId) return sessions[i];
        }
    ...
    function startMultiplayerGame(data) {
        data = data || {};
        if (window.Scene_Map && SceneManager._scene instanceof Scene_Map) return;
        DataManager.setupNewGame();
        if (data.mapId && window.$gamePlayer) {
            $gamePlayer.reserveTransfer(Number(data.mapId), Number(data.x || 0), Number(data.y || 0));
        }
    ...
    Net.connect = function(url, name, roomCode, retryCount, retryGeneration) {
        if (!window.WebSocket) {
        ...
        if (!retryGeneration) {
            Net._connectGeneration = (Net._connectGeneration || 0) + 1;
            retryGeneration = Net._connectGeneration;
        }
        ...
        } else if (retryCount === 1 || retryCount === 4 || retryCount === 8) {
            notify("Connexion en attente", [
                "Le tunnel Cloudflare n'est pas encore pret.",
                "Nouvelle tentative " + (retryCount + 1) + "/12."
            ], "network");
        }
        ...
        socket.onclose = function(event) {
            ...
            Net._lastLocalStateForVelocity = null;
            Net._lastMoveSignature = "";
            setStatus("offline", { playerId: "", roomId: "", joinUrl: "", isHost: false, roomMeta: null });
            if (wasConnected) notify("Session fermee", [event.reason || "Connexion interrompue."], "network");
        };

        socket.onerror = function() {
            if (Net.socket !== socket) return;
            if (isEphemeralTunnelUrl(url) && retryCount < 11 && Net._connectGeneration === retryGeneration) {
                var delay = Math.min(9000, 1500 + retryCount * 1000);
                setStatus("connecting", { lastError: "Tunnel Cloudflare en cours d'activation." });
                socket.onclose = null;
                socket.onerror = null;
                try {
                    socket.close();
                ...
                setTimeout(function() {
                    if (Net._connectGeneration !== retryGeneration) return;
                    Net.connect(url, name, roomCode, retryCount + 1, retryGeneration);
                }, delay);
                return;
            }
    ...
    Net.setLobbyServerUrl = function(url) {
        url = urlWithoutRoom(url || Net.lobby.serverUrl || Net.state.remoteUrl || config.remoteUrl).replace(/\/$/, "");
        Net.lobby.serverUrl = url;
        Net.setRemoteUrl(url);
        storageSet("l3-net-lobby-url", url);
        return url;
    };

    Net.refreshLobby = function(callback) {
        var url = Net.setLobbyServerUrl(Net.lobby.serverUrl || Net.state.remoteUrl || config.remoteUrl);
        var roomsUrl = roomsHttpUrl(url);
        Net.lobby.status = "loading";
        Net.lobby.lastError = "";

        httpJson(roomsUrl, function(error, data) {
            if (error) {
                Net.lobby.status = "error";
                Net.lobby.lastError = error.message || String(error);
                Net.lobby.sessions = [];
                if (callback) callback(error);
                emit("lobby", Net.lobby);
                return;
            }
            Net.lobby.status = "ready";
            Net.lobby.sessions = normalizeSessions(data && data.rooms ? data.rooms : []);
            Net.lobby.lastRefreshAt = Date.now();
            if (!Net.lobby.selectedRoomId && Net.lobby.sessions[0]) Net.lobby.selectedRoomId = Net.lobby.sessions[0].roomId;
            if (callback) callback(null, Net.lobby.sessions);
            emit("lobby", Net.lobby);
        });
    };

    Net.createLobbySession = function(options) {
        options = options || {};
        var name = options.playerName || askPlayerName();
        if (!name) return false;
        Net.setPlayerName(name, true);

        var room = normalizeRoom(options.roomCode || makeRoomCode());
        var title = cleanSessionTitle(options.title || window.prompt("Nom de la partie", name + " - Partie"));
        if (!title) title = name + " - Partie";
        var maxInput = options.maxPlayers || window.prompt("Nombre de joueurs max (2-4)", String(config.maxPlayers || 4));
        if (maxInput === null) return false;
        var maxPlayers = Math.max(2, Math.min(4, Number(maxInput || config.maxPlayers || 4)));
        Net.lobby.pendingHostRoom = {
            title: title,
            maxPlayers: maxPlayers,
            status: "waiting",
            private: false
        };
        Net.state.isHost = true;
        Net.setRoom(room);

        var info = Net.loadTunnelUrl(true);
        if (info && info.provider === "direct") {
            var hostUrl = info.hostUrl || info.localUrl || config.defaultUrl;
            var publicUrl = info.websocketUrl || info.publicUrl || Net.state.remoteUrl || config.remoteUrl;
            Net.state.joinUrl = urlWithRoom(publicUrl, room);
            Net.setRemoteUrl(publicUrl);
            Net.setLobbyServerUrl(publicUrl);
            return Net.connect(hostUrl, name, room);
        }

        notify("Serveur introuvable", [
            "Lance npm run net:start:remote avant de creer une partie.",
            "Le fichier L3NetServer/remote-url.json doit etre present."
        ], "error");
        return false;
    };

    Net.joinLobbySession = function(session) {
        session = session || selectedSession();
        if (!session) {
            notify("Aucune partie", ["Selectionne une session disponible."], "error");
            return false;
        }
        if (session.status === "full") {
            notify("Session pleine", ["Cette partie n'a plus de slot libre."], "error");
            return false;
        }
        if (session.status === "in_game") {
            notify("Partie deja lancee", ["La synchronisation en cours de partie arrivera dans une prochaine etape."], "error");
            return false;
        }
        var name = askPlayerName();
        if (!name) return false;
        Net.state.isHost = false;
        Net.state.roomMeta = session;
        Net.setPlayerName(name, true);
        Net.setRoom(session.roomId);
        return Net.connect(Net.lobby.serverUrl || Net.state.remoteUrl || config.remoteUrl, name, session.roomId);
    };

    Net.startLobbyGame = function() {
        if (!Net.isConnected()) {
            notify("Session absente", ["Cree ou rejoins une partie avant de lancer."], "error");
            return false;
        }
        if (!Net.state.isHost) {
            notify("Action reservee", ["Seul l'hote peut lancer la partie."], "error");
            return false;
        }
        return Net.send("startGame", {
            mapId: $dataSystem ? $dataSystem.startMapId : 0,
            x: $dataSystem ? $dataSystem.startX : 0,
            y: $dataSystem ? $dataSystem.startY : 0
        });
    };

    Net.setLobbyReady = function(ready) {
        if (!Net.isConnected()) return false;
        return Net.send("setReady", { ready: ready !== false });
    };
    ...
    Net.startOnlineHost = function(roomCode) {
        ...
        if (existingTunnel && existingTunnel.provider === "direct") {
            var hostUrl = existingTunnel.hostUrl || existingTunnel.localUrl || config.defaultUrl;
            var publicUrl = existingTunnel.websocketUrl || existingTunnel.publicUrl || Net.state.remoteUrl || config.remoteUrl;
            var joinUrl = existingTunnel.joinUrl || urlWithRoom(publicUrl, existingTunnel.room || roomCode);
            Net.state.joinUrl = joinUrl;
            Net.state.tunnelUrl = "";
            Net.setRemoteUrl(publicUrl);
            if (existingTunnel.room) Net.setRoom(existingTunnel.room);
            if (Net.connect(hostUrl, playerName(), existingTunnel.room || roomCode)) {
                Net.state.joinUrl = joinUrl;
                return true;
            }
        }
    ...
    Net.disconnect = function(silent) {
        if (!silent) Net._connectGeneration = (Net._connectGeneration || 0) + 1;
        if (Net.socket) {
            try {
    ...
    function handlePacket(raw) {
        ...
        if (type === "welcome") {
            ...
            Net.state.roomMeta = data.room || Net.state.roomMeta || null;
            Net.state.players = {};
            mergePlayers(data.players || []);
            setStatus("connected");
            notify("Session rejointe", ["Code : " + Net.state.roomCode, "ID joueur : " + Net.state.playerId], "network");
            if (Net.lobby.pendingHostRoom) {
                Net.send("hostRoom", Net.lobby.pendingHostRoom);
                Net.send("setReady", { ready: true });
                Net.lobby.pendingHostRoom = null;
            }
            sendPlayerState(true);
            ...
            Net.state.roomMeta = data || null;
            if (data && data.maxPlayers) Net.state.maxPlayers = Number(data.maxPlayers || Net.state.maxPlayers || 4);
            if (data && data.playersList) {
                Net.state.players = {};
                mergePlayers(data.playersList);
            }
            emit("room", Net.state.roomMeta);
        } else if (type === "gameStart") {
            Net.state.pendingGameStart = data || {};
    ...
    function localPlayerState() {
        if (!window.$gamePlayer || !window.$gameMap || !window.$dataMap) return null;
        if (window.SceneManager && window.Scene_Map && !(SceneManager._scene instanceof Scene_Map)) return null;
    ...
    function lobbySignature() {
        var sessions = (Net.lobby.sessions || []).map(function(session) {
            return [
                session.roomId,
                session.title,
                session.hostName,
        ...
        return [
            ...
            Net.lobby.selectedRoomId,
            sessions,
            Net.state.isHost ? 1 : 0,
            Net.state.roomMeta ? JSON.stringify(Net.state.roomMeta) : ""
        ].join("~");
    }

    function renderTitleLobbyScene(scene) {
        L3UI.clear(scene);
        L3UI.hideNativeWindows(scene);
        scene._l3NetLobbySignature = lobbySignature();

        var connected = Net.isConnected();
        var screen = L3UI.Layout.screen(scene, { className: "l3-net-title-lobby-page" });
        var shell = L3UI.el("main", { className: "l3-net-title-lobby-shell l3-panel" });
        screen.appendChild(shell);

        shell.appendChild(L3UI.el("header", {
            className: "l3-net-title-lobby-head",
            html:
                "<div>" +
                    "<span class=\"l3-net-kicker\">Multijoueur</span>" +
                    "<h1>" + L3UI.escapeHtml(connected ? "Salle d'attente" : "Parties disponibles") + "</h1>" +
                    "<p>" + L3UI.escapeHtml(connected ? "Attends tes amis, puis lance la partie quand tout le monde est pret." : "Cree une partie ou rejoins une session ouverte depuis l'ecran titre.") + "</p>" +
                "</div>" +
                "<strong>" + L3UI.escapeHtml(playerName(Net.state.name || "")) + "</strong>"
        }));

        if (connected) renderWaitingRoom(scene, shell);
        else {
            renderSessionBrowser(scene, shell);
            if (Net.lobby.status === "idle" && !Net.lobby.lastRefreshAt) {
                setTimeout(function() {
                    Net.refreshLobby(function() {
                        if (scene && SceneManager._scene === scene) renderTitleLobbyScene(scene);
                    });
                }, 80);
            }
        }
    ...
    function renderSessionBrowser(scene, shell) {
        var serverUrl = Net.lobby.serverUrl || Net.state.remoteUrl || config.remoteUrl;
        shell.appendChild(L3UI.el("section", {
            className: "l3-net-lobby-server-card",
            html:
                "<span>Serveur</span>" +
                "<strong>" + L3UI.escapeHtml(serverUrl || "-") + "</strong>" +
                "<em>" + L3UI.escapeHtml(lobbyStatusText()) + "</em>"
        }));

        var content = L3UI.el("section", { className: "l3-net-lobby-content" });
        var sessionPanel = L3UI.el("div", { className: "l3-net-session-list-panel" });
        sessionPanel.appendChild(L3UI.el("div", {
            className: "l3-net-section-title",
            html: "<span>Sessions</span><strong>" + L3UI.escapeHtml(String((Net.lobby.sessions || []).length)) + "</strong>"
        }));

        var sessionItems = (Net.lobby.sessions || []).map(function(session) {
            return {
                label: session.title,
                detail: session.hostName + " - " + session.players + " / " + session.maxPlayers + " joueur(s)",
                right: sessionStatusLabel(session),
                enabled: session.status !== "full",
                handler: function() {
                    Net.lobby.selectedRoomId = session.roomId;
                    Net.joinLobbySession(session);
                    renderTitleLobbyScene(scene);
                }
            };
        });
        if (!sessionItems.length) {
            sessionPanel.appendChild(L3UI.el("div", {
                className: "l3-net-empty",
                text: Net.lobby.status === "error" ? Net.lobby.lastError : "Aucune partie ouverte sur ce serveur."
            }));
        } else {
            var sessions = L3UI.Components.CommandList({
                id: "l3-net-title-sessions",
                scene: scene,
                items: sessionItems,
                onCancel: function() { SceneManager.pop(); }
            });
            sessions.element.className += " l3-net-title-session-list";
            sessionPanel.appendChild(sessions.element);
            L3UI.Focus.activate(sessions.group);
        }
        ...
        var actionItems = [
            {
                label: "Actualiser",
                icon: "R",
                detail: "Recharge la liste.",
                handler: function() {
                    Net.refreshLobby(function() { renderTitleLobbyScene(scene); });
                }
            },
            {
                label: "Adresse serveur",
                icon: "@",
                detail: "IP ou domaine de l'hote.",
                handler: function() {
                    var value = window.prompt("Adresse du serveur", serverUrl || config.remoteUrl);
                    if (!value) return;
                    Net.setLobbyServerUrl(value);
                    Net.refreshLobby(function() { renderTitleLobbyScene(scene); });
                }
            },
            {
                ...
                icon: "+",
                detail: "Ouvre une salle d'attente.",
                handler: function() {
                    Net.createLobbySession();
                    renderTitleLobbyScene(scene);
                }
            },
            {
                label: "Retour",
                icon: "<",
                handler: function() { SceneManager.pop(); }
            }
        ];

        var actions = L3UI.Components.CommandList({
            id: "l3-net-title-actions",
            scene: scene,
            items: actionItems,
            onCancel: function() { SceneManager.pop(); }
        });
        actions.element.className += " l3-net-title-actions";

        content.appendChild(sessionPanel);
        content.appendChild(actions.element);
        shell.appendChild(content);
        if (!sessionItems.length) L3UI.Focus.activate(actions.group);
    }

    function renderWaitingRoom(scene, shell) {
        var room = Net.state.roomMeta || {};
        var link = Net.state.joinUrl || urlWithRoom(Net.state.remoteUrl || Net.lobby.serverUrl || config.remoteUrl, Net.state.roomCode);
        shell.appendChild(L3UI.el("section", {
            className: "l3-net-waiting-card",
            html:
                "<span>Session</span>" +
                "<strong>" + L3UI.escapeHtml(room.title || Net.state.roomCode || "Partie") + "</strong>" +
                "<em>" + L3UI.escapeHtml(link || "Lien en preparation") + "</em>"
        }));

        var playersPanel = L3UI.el("section", { className: "l3-net-waiting-players" });
        playersPanel.appendChild(L3UI.el("div", {
            className: "l3-net-section-title",
            html:
                "<span>Joueurs</span>" +
                "<strong>" + L3UI.escapeHtml(String(1 + Net.players().length)) + " / " + L3UI.escapeHtml(String(Net.state.maxPlayers || config.maxPlayers)) + "</strong>"
        }));
        var grid = L3UI.el("div", { className: "l3-net-player-grid" });
        grid.appendChild(L3UI.el("section", {
            className: "l3-net-player-card is-local",
            html:
                "<strong>" + L3UI.escapeHtml(playerName(Net.state.name || "")) + "</strong>" +
                "<span>" + L3UI.escapeHtml(Net.state.isHost ? "Hote" : "Joueur") + "</span>"
        }));
        var players = Net.players();
        for (var i = 0; i < players.length; i++) grid.appendChild(playerCard(players[i]));
        playersPanel.appendChild(grid);
        shell.appendChild(playersPanel);
        ...
        if (Net.state.isHost) {
            items.push({
                ...
                icon: ">",
                detail: "Tous les joueurs partent sur la map.",
                handler: function() { Net.startLobbyGame(); }
            });
        } else {
            items.push({
                label: "Pret",
                icon: "V",
                detail: "Indique que tu es pret.",
                handler: function() { Net.setLobbyReady(true); }
            });
        }
        items.push({
            label: "Copier le lien",
            icon: "C",
            detail: "Pour inviter un ami.",
            handler: function() { copyText(link); }
        });
        items.push({
            label: "Changer mon nom",
            icon: "N",
            detail: playerName(Net.state.name || ""),
            handler: function() {
                var nextName = askPlayerName();
                if (!nextName) return;
                Net.rename(nextName);
        ...
        actions.element.className += " l3-net-title-actions";
        shell.appendChild(actions.element);
        L3UI.Focus.activate(actions.group);
    }

    function lobbyStatusText() {
        if (Net.lobby.status === "loading") return "Recherche des parties...";
        if (Net.lobby.status === "error") return Net.lobby.lastError || "Serveur inaccessible.";
        if (Net.lobby.lastRefreshAt) return "Mis a jour.";
        return "Pas encore actualise.";
    }

    function sessionStatusLabel(session) {
        if (!session) return "-";
        if (session.status === "in_game") return "En jeu";
        if (session.status === "full") return "Pleine";
        return "En attente";
    }
    ...
    function installTitleCommand() {
        if (!config.titleLobby || !window.L3UI || !L3UI.Slots) return;
        L3UI.Slots.add("title.commands", "l3-net-title", {
            label: config.titleLobbyLabel,
            right: ">",
            order: 30,
            handler: function() {
                if (L3UI.Scenes) L3UI.Scenes.push("l3NetTitleLobby");
                else openScene();
            }
        });
    }
    ...
    function installCss() {
        ...
        L3UI.addCss("l3-net-core-css",
            ...
            ".l3-net-title-lobby-page{display:flex;align-items:center;justify-content:center;padding:32px;background:linear-gradient(125deg,rgba(8,13,24,.96),rgba(8,13,24,.78));}" +
            ".l3-net-title-lobby-shell{width:min(1040px,calc(100vw - 64px));max-height:calc(100vh - 64px);overflow:auto;padding:28px;display:flex;flex-direction:column;gap:16px;}" +
            ".l3-net-title-lobby-head{display:flex;align-items:flex-start;justify-content:space-between;gap:24px;padding-bottom:14px;border-bottom:1px solid rgba(148,163,184,.16);}" +
            ".l3-net-title-lobby-head h1{margin:4px 0 8px;font-size:38px;line-height:1.05;color:#f8fafc;}" +
            ".l3-net-title-lobby-head p{margin:0;max-width:720px;color:#b6c4d6;line-height:1.45;}" +
            ".l3-net-title-lobby-head>strong{padding:7px 12px;border:1px solid rgba(56,189,248,.3);border-radius:999px;color:#7dd3fc;background:rgba(14,165,233,.08);}" +
            ".l3-net-lobby-server-card,.l3-net-waiting-card{display:flex;flex-direction:column;gap:8px;padding:16px;border:1px solid rgba(56,189,248,.24);border-radius:8px;background:rgba(14,165,233,.09);}" +
            ".l3-net-lobby-server-card span,.l3-net-waiting-card span{font-size:11px;text-transform:uppercase;color:#7dd3fc;}" +
            ".l3-net-lobby-server-card strong,.l3-net-waiting-card strong{font-size:18px;color:#f8fafc;word-break:break-all;}" +
            ".l3-net-lobby-server-card em,.l3-net-waiting-card em{font-style:normal;color:#9fb0c4;word-break:break-all;}" +
            ".l3-net-lobby-content{display:grid;grid-template-columns:minmax(0,1fr) 280px;gap:16px;align-items:start;}" +
            ".l3-net-session-list-panel,.l3-net-waiting-players{display:flex;flex-direction:column;gap:12px;padding:16px;border:1px solid rgba(148,163,184,.16);border-radius:8px;background:rgba(15,23,42,.44);}" +
            ".l3-net-section-title{display:flex;align-items:center;justify-content:space-between;gap:16px;color:#9fb0c4;}" +
            ".l3-net-section-title span{font-size:11px;text-transform:uppercase;}" +
            ".l3-net-section-title strong{color:#f8fafc;}" +
            ".l3-net-title-session-list{display:grid;gap:10px;}" +
            ".l3-net-title-session-list .l3-command{min-height:70px;align-items:flex-start;justify-content:center;}" +
            ".l3-net-title-actions{display:grid;gap:10px;}" +
            ".l3-net-title-actions .l3-command{min-height:66px;align-items:flex-start;justify-content:center;}" +
            ".l3-net-title-actions .l3-command .l3-command-detail,.l3-net-title-session-list .l3-command .l3-command-detail{white-space:normal;line-height:1.35;}" +
            ".l3-net-waiting-card strong{font-size:26px;}" +
            ".l3-net-player-card.is-local{border-color:rgba(56,189,248,.45);background:linear-gradient(135deg,rgba(14,165,233,.18),rgba(30,41,59,.72));}" +
            "@media (max-width: 840px){.l3-net-title-lobby-page{padding:16px}.l3-net-title-lobby-shell{width:calc(100vw - 32px);max-height:calc(100vh - 32px);padding:18px}.l3-net-title-lobby-head{flex-direction:column}.l3-net-lobby-content{grid-template-columns:1fr}.l3-net-title-lobby-head h1{font-size:31px;}}"
        );
    }
    ...
    Scene_Title.prototype.start = function() {
        ...
        if (Net.isConnected()) Net.disconnect(true);
    };
    ...
    if (window.L3UI && L3UI.Scenes) {
        L3UI.Scenes.register("l3NetTitleLobby", {
            title: config.titleLobbyLabel,
            render: renderTitleLobbyScene,
            update: function(scene) {
                if (Input.isTriggered("cancel") || TouchInput.isCancelled()) SceneManager.pop();
                sendNetworkPing(false);
                if (!Net.isConnected() && Graphics.frameCount % 180 === 0 && Net.lobby.status !== "loading") {
                    Net.refreshLobby(function() {
                        if (scene && SceneManager._scene === scene) renderTitleLobbyScene(scene);
                    });
                }
                if (Graphics.frameCount % 15 === 0 && scene) {
                    var signature = lobbySignature();
                    if (scene._l3NetLobbySignature !== signature) renderTitleLobbyScene(scene);
                }
            }
        });
