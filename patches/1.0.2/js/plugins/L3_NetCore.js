/*:
 * @plugindesc L3 Net Core v0.3 - Sessions locales et distantes 2-4 joueurs pour RPG Maker MV.
 * @author L3ViKk
 * @version 0.3.0
 *
 * @param Enabled
...
(function() {
    ...
    var config = {
        ...
        sendInterval: Math.max(1, readNumber("Position Send Interval", 3)),
        heartbeatInterval: Math.max(10, readNumber("Position Heartbeat Interval", 30)),
        interpolationDelay: Math.max(0, readNumber("Interpolation Delay", 120)),
        remoteSnapDistance: Math.max(1, readNumber("Remote Snap Distance", 3)),
    ...
    Net.state.remoteUrl = storageGet("l3-net-remote-url", config.remoteUrl);
    ...
    Net.connect = function(url, name, roomCode) {
        ...
        socket.onerror = function() {
            ...
            notify("Erreur reseau", ["Le serveur ne repond pas ou l'adresse est incorrecte."], "error");
        };
    ...
    Net.startOnlineHost = function(roomCode) {
        ...
        if (Net.connectTunnel(roomCode, true)) return true;
    ...
    Net.setRemoteUrl = function(url) {
        ...
        storageSet("l3-net-remote-url", Net.state.remoteUrl);
    ...
    function handlePacket(raw) {
        ...
        if (type === "welcome") {
            ...
            setRemoteState(data.playerId, data.name, data.state, data.time || packet.serverTime);
    ...
    function setRemoteState(playerId, name, state, serverTime) {
        if (!playerId || playerId === Net.state.playerId) return;
        var player = Net.state.players[playerId] || { playerId: playerId };
        ...
        character.applyPlayer(player, serverTime);
    ...
    function sendPlayerState(force) {
        ...
        if (!force && frame - Net._lastSentFrame < config.sendInterval) return;
    ...
    Game_L3NetRemote.prototype.applyPlayer = function(player, serverTime) {
        ...
        if (state.pattern !== undefined) this.setPattern(Number(state.pattern || 1));
    ...
    Game_L3NetRemote.prototype.updateNetInterpolation = function() {
        ...
        var renderAt = Date.now() - config.interpolationDelay;
        ...
        if (to) {
            ...
            targetRealX = this._realX + (from.realX - this._realX) * 0.35;
            targetRealY = this._realY + (from.realY - this._realY) * 0.35;
        }
        ...
        if (Math.abs(this._realX - targetRealX) > config.remoteSnapDistance || Math.abs(this._realY - targetRealY) > config.remoteSnapDistance) {
            ...
            this._realX = targetRealX;
            this._realY = targetRealY;
            this._x = Math.round(targetRealX);
            this._y = Math.round(targetRealY);
        }
    ...
    function updateSpriteset(spriteset) {
        ...
        for (var playerId in players) {
            ...
            if (!spriteset._l3NetSprites[playerId]) {
                spriteset._l3NetSprites[playerId] = new Sprite_Character(character);
