/*:
 * @plugindesc L3 Net Core v0.4 - Sessions locales et distantes 2-4 joueurs pour RPG Maker MV.
 * @author L3ViKk
 * @version 0.4.0
 *
 * @param Enabled
...
(function() {
    ...
    function smoothProfile() {
        if (config.smoothProfile === "local") {
            return { minDelay: 60, maxDelay: 150, smooth: 0.72, predict: Math.min(config.predictionLimit, 90) };
        }
        if (config.smoothProfile === "remote") {
            return { minDelay: 100, maxDelay: 260, smooth: 0.58, predict: Math.min(config.predictionLimit, 160) };
        }
        return { minDelay: 140, maxDelay: 380, smooth: 0.48, predict: config.predictionLimit };
    }
