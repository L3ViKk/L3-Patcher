...
function startTunnel(cloudflared) {
    ...
    function handleOutput(data) {
        ...
        if (match) prepareTunnelInfo(match[0]).catch(function(error) {
            console.error("[L3Online] Le tunnel Cloudflare n'est pas joignable : " + (error && error.message ? error.message : error));
            cleanup();
            process.exit(1);
        });
    }
...
async function prepareTunnelInfo(url) {
    if (tunnelDiscovered) return;
    tunnelDiscovered = true;
    console.log("[L3Online] Lien Cloudflare recu, verification DNS et serveur...");
    await waitForPublicTunnel(url, 60000);
    writeTunnelInfo(url);
}
